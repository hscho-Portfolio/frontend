...
| 빌드 | Vite + `@hono/vite-build/vercel` (Vercel Build Output API) |
...
│       ├── admin-project-form.tsx # 프로젝트 등록/수정 폼 (Full Preview 포함)
...
│   │   ├── admin.css      # 어드민 전체 (로그인 / 폼 / 모달 / 업로더 / toast / confirm)
...
`alert()` / `confirm()` / `prompt()` 대신 커스텀 UI를 사용합니다.

- `toast(message, type)` — 우하단 알림 (`success` / `error` / `warn` / `info`)
- `confirmModal({ title, message, dangerous })` — 삭제 등 위험 동작 전 확인 모달

### 프로젝트 Full Preview

프로젝트 폼 우상단 **Full Preview** 버튼을 클릭하면 macOS 데스크톱 환경을 모사한 전체 화면 미리보기가 열립니다.  
폼에 입력한 제목, 요약, 상태, 기간, 썸네일, 기능, 스택, 아키텍처, 회고가 실제 프로젝트 상세 페이지와 동일한 레이아웃으로 표시됩니다.

### 이미지 업로드

- **썸네일**: 프로젝트 폼 사이드바 Thumbnail 영역에서 업로드. 데스크톱 카드와 상세 Hero에 표시됩니다.
- **아키텍처 이미지**: 프로젝트 폼 Architecture Upload 영역에서 업로드. 프로젝트 상세 Architecture 섹션에 표시됩니다.
- 업로드 성공/실패 시 toast 알림이 표시됩니다.

> **주의**: `APP_ENV=local` 환경에서 업로드된 파일은 Docker named volume(`portfolio_os_uploads`)에 저장됩니다.  
> `docker compose down -v`로 볼륨을 삭제하면 업로드 파일도 함께 삭제됩니다.

### 스택 아이콘 입력 방법

스택 추가 시 **Icon URL or class** 필드에 두 가지 방식 중 하나를 입력합니다.

| 방식 | 예시 |
|---|---|
| Font Awesome 클래스 | `fa-brands fa-react`, `fa-solid fa-database` |
| 이미지 URL (devicons 추천) | `https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg` |
