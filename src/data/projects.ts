// CHO OS — Project catalog (mock)
//
// 추후 D1 DB의 `projects` 테이블에서 fetch 하도록 교체 예정.
import type { Project, TechStack } from './types'
import { pickStacks } from './stacks'

/** 프로젝트별 stack에 usage(사용 설명)를 덮어쓰는 헬퍼 */
const withUsage = (stacks: TechStack[], usageMap: Record<string, string>): TechStack[] =>
...
export const PROJECTS: Project[] = [
  {
    ...
    stacks: withUsage(
      pickStacks('Next.js', 'TypeScript', 'Tailwind CSS', 'FastAPI', 'PostgreSQL', 'AWS', 'Docker'),
      {
        'Next.js': 'OS형 화면 구성, 라우팅, 프로젝트 상세 창 렌더링',
        FastAPI: '관리자 CMS API, 프로젝트 CRUD, 이미지 업로드 API 구현',
        PostgreSQL: '프로젝트, 기술스택, 카테고리, 관리자 데이터 저장',
        AWS: 'ECS Fargate에 FastAPI 배포, S3에 정적 파일 호스팅',
        Docker: '백엔드 컨테이너화 및 일관된 배포 환경 구성',
      }
    ),
    architecture: [
      '사용자',
  ...
  },
  {
    ...
    stacks: withUsage(pickStacks('Python', 'Scikit-learn', 'XGBoost'), {
      Python: '데이터 전처리와 feature 엔지니어링',
      XGBoost: 'CE / EQE 예측 회귀 모델 학습',
      'Scikit-learn': 'baseline 비교 및 평가 지표 계산',
    }),
  ...
  },
  {
    ...
    stacks: withUsage(pickStacks('Python', 'Scikit-learn', 'FastAPI'), {
      Python: '추천 로직 및 데이터 처리',
      FastAPI: '추천 API 서버 구현',
      'Scikit-learn': '컨텐츠 기반 유사도 계산',
    }),
  ...
  },
  {
    ...
    stacks: pickStacks('Python', 'Scikit-learn'),
  ...
  },
  {
    ...
    stacks: pickStacks('TypeScript', 'React'),
    architecture: ['User Wallet', 'Frontend dApp', 'Smart Contract', 'Blockchain Network'],
    retrospective: ['실제 투표 환경에서 요구되는 비밀성과 검증 가능성 사이의 트레이드오프를 깊이 고민했다.'],
    links: { github: '#' },
  },
  {
    slug: 'ai-drawing',
    ...
    stacks: withUsage(pickStacks('Python', 'FastAPI', 'AWS'), {
      AWS: 'S3 이미지 저장 및 모델 서버 호스팅',
      FastAPI: '이미지 생성 요청 API',
      Python: '모델 추론 파이프라인',
    }),
