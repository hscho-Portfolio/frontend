...
import type { Project, TechStack, StackCategory, Award, Shortcut, ApiProjectList, ApiProjectDetail } from './types'
...
export async function fetchShortcuts(): Promise<Shortcut[]> {
  try {
    const res = await fetch(`${BACKEND_URL}/api/v1/public/shortcuts`, {
      headers: { Accept: 'application/json' },
